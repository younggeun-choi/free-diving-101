...
앱이 백그라운드로 전환되었다가 포그라운드로 복귀할 때 다음과 같이 동작합니다:

**타이머 동작**:
- Timestamp 기반 타이머 사용 (`phaseEndsAtRef = Date.now() + remainingMs`)
- 백그라운드 진입: JS `setInterval` 중단 (불필요한 작업 방지)
- 포그라운드 복귀: 실제 경과 시간 계산 후 타이머 동기화
- Phase overflow 처리: 경과 시간이 현재 phase를 초과하면 다음 phase로 자동 전환

**TTS 동작**:
- 백그라운드 진입: `stopSpeech()` 호출하여 iOS synthesizer 중단
- 포그라운드 복귀: synthesizer 재시작 후 현재 상태 즉시 안내
- 예: Breathe phase가 끝나고 Hold로 전환되었다면 "Hold" 발화

**Phase 전환 예시**:
```
1. Round 1 Breathe 시작 (120초)
2. 90초 경과 후 백그라운드 전환 (30초 남음)
3. 40초 후 포그라운드 복귀 (총 130초 경과)
4. 결과: Hold phase로 전환, 10초 overflow 차감
   → Hold 90초 중 80초부터 시작
```

**구현 세부사항**:
- `AppState` 리스너로 백그라운드/포그라운드 감지
- `reconcilePhaseAfterResume()`: 경과 시간 계산 및 phase 동기화
- `completePhase(carryMs)`: overflow 시간을 다음 phase로 전달
- 여러 phase를 연속으로 건너뛸 수 있음 (재귀적 처리)

**제약사항**:
- iOS/Android에서 백그라운드 중 JS 실행 제한
- 백그라운드에서 TTS는 기본적으로 동작하지 않음
- 로컬 알림은 별도 구현 필요 (현재 미지원)
...
| 2.1 | 2025-11-10 | 백그라운드/포그라운드 동작 섹션 추가 (8.4) | Claude |
